...
async def test_stdio_responses_search_citations_error_and_survival():
    request_bodies = []
    post_calls = 0
    ...
    upstream = ThreadingHTTPServer(("127.0.0.1", 0), ResponsesHandler)
    thread = threading.Thread(target=upstream.serve_forever, daemon=True)
    thread.start()
    root = Path(__file__).resolve().parents[1]
    env = os.environ.copy()
    ...
    env["GROK_API_KEY"] = "grok-secret-key"
    env["GROK_PRIMARY_MODEL"] = "primary"
    env["GROK_API_PROTOCOL"] = "responses"
    env["GROK_MODEL_MAX_ATTEMPTS"] = "1"
    env["TAVILY_ENABLED"] = "false"
    server = StdioServerParameters(
        command=sys.executable,
        ...
        async with stdio_client(server) as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                sources = await session.call_tool(
                    "get_sources", {"session_id": success.structuredContent["session_id"]}
                )
                failed = await session.call_tool("web_search", {"query": "incomplete"})
                config_after_error = await session.call_tool("get_config_info", {})
    finally:
        upstream.shutdown()
        upstream.server_close()
        thread.join(timeout=2)
    ...
    assert success.structuredContent["status"] == "success"
    assert success.structuredContent["content"] == "Responses answer"
    assert sources.structuredContent["sources"] == [
        {
            "url": "https://example.com/source",
            "title": "Source",
            "description": None,
    ...
    assert failed.structuredContent["status"] == "error"
    assert failed.structuredContent["grok_error"]["last_error_type"] == "response_incomplete"
    assert config_after_error.structuredContent["status"] == "success"
    assert request_bodies[0]["tools"] == [{"type": "web_search"}]
