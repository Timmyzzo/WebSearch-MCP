...
- 单次 Grok HTTP 读取超时为 120 秒。
- 可恢复错误默认最多真实调用 5 次。
- Cherry Studio MCP 工具外层超时计划调整为 300 秒。

如果没有整次调用的总时间预算，最坏情况可能超过 5 × 120 秒并叠加退避等待，远大于 Cherry Studio 的 300 秒。因此，仅把客户端超时改成 300 秒不能独立解决问题。
...
- `GROK_MODEL_MAX_ATTEMPTS=5` 继续表示最多 5 次真实请求。
- 每次重试前必须检查剩余总时间；剩余时间不足以完成一次合理请求时应停止重试。
- 认证错误、参数错误、模型不存在和模型无权限继续立即停止。
- 模型暂时不可用、429、5xx、连接错误、读取超时和流中断仍可重试，但不得突破总预算。
...
- Grok 单次读取上限仍为 120 秒，但每次请求会裁剪到当前剩余总预算；剩余预算不足时不开始新重试。
