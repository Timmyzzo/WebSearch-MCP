...
- 来源会话、规划会话 TTL/LRU 与模型目录 TTL 刷新。
...
- 层 B 短英文搜索提示：广而快、可溯源；复杂问题再多视角/深挖；禁止硬编码 7–16 次检索楼层。客户端证据标准见 `CLIENT_SYSTEM_PROMPT.md`。
...
- 来源会话最多 256 项、1 小时 TTL；规划会话最多 256 项、空闲 1 小时 TTL；两者均为进程内 LRU。模型目录成功结果缓存 5 分钟，失败不缓存。
