...
| `GROK_RETRY_MULTIPLIER` | 否 | `1` | 指数退避初始乘数，允许大于或等于 0 的小数。 |
| `GROK_RETRY_MAX_WAIT` | 否 | `10` | 单次退避等待上限秒数，允许大于或等于 0。 |
| `GROK_RETRYABLE_UPSTREAM_CODES` | 否 | 见下文 | HTTP 200 错误体中仍应重试的上游错误码，支持逗号、分号或换行分隔。 |
