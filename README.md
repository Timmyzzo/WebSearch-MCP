...
MCP Client ──stdio──► WebSearch MCP
                       ├─ web_search ─► Grok Chat/Responses + 可选 Tavily 信源
                       ├─ get_sources ─► 搜索信源缓存
...
| 可审计协议 | 可选 xAI Responses 显式 `web_search`，解析 citations、annotations 和搜索轨迹；默认 Chat 模式保持兼容。 |
...
- 外部项目代码审计、可选 Responses 协议和有界运行时缓存：已完成。
- 下一阶段：P6 跨客户端真实人工验收。
...
| `GROK_API_PROTOCOL` | 否 | `chat_completions` | Grok 请求协议；可设为 `responses` 以显式调用服务端 `web_search`。 |
| `GROK_RESPONSES_MAX_TOOL_CALLS` | 否 | `16` | Responses 单次最多服务端工具调用数，范围 7–32。 |
...
默认 `GROK_API_PROTOCOL=chat_completions`，因此现有只支持 `/chat/completions` 的中转站无需改动。设置为 `responses` 后，服务改用非流式 `/responses` 和显式 `web_search`；只有上游 `status=completed` 且答案非空才成功。结构化 citations、inline annotations、search/open-page URL 会进入 `get_sources`，请求固定发送 `store=false`，避免 xAI 官方默认保存请求/响应。Responses 是否可用仍取决于所配置端点和模型。
...
## Grok 协议、单强模型与最多五次真实尝试

Chat Completions 是兼容默认值；Responses 是可选的可审计路径。两种协议共用同一套单强模型、并发、总预算、错误分类和真实尝试计数，不会在失败时自动切换协议。OpenRouter 在 Chat 模式保留兼容的 `:online` 后缀；Responses 模式使用 `openrouter:web_search`，不追加该后缀。
...
### Responses 返回 400/422 或工具不可用

先恢复 `GROK_API_PROTOCOL=chat_completions`。Responses 是端点能力，不是所有 OpenAI 兼容中转站都支持；确认服务提供 `/responses`、当前模型支持服务端 `web_search`，并检查 `GROK_RESPONSES_MAX_TOOL_CALLS` 是否在 7–32 范围内。
